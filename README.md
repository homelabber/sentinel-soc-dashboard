...
> All screenshots use **synthetic data** generated by `tools/mock-server.py`.
> Incident titles, analyst names and tenant identifiers are fabricated — a
> dashboard of real SOC state is not something to publish.

**Live view** — one 1080p screen, no scrolling. Tiles, a High/Critical feed with
SLA warnings, severity split, ingestion volume, analyst workload, incident age,
and the full 24h feed.

![Live view, dark theme](docs/img/dashboard-dark.png)

**Trends** — 30-day incident volume by severity, mean time to resolve, SLA
compliance, and open backlog.

![Trends view](docs/img/trends.png)

**Incident feed** — every row links straight to the incident in the Microsoft
Defender portal. Rows past their severity SLA are flagged (Critical 2h, High 8h).

![Incident feed close-up](docs/img/incident-feed.png)

**Light theme** — same layout, for rooms that aren't dark.

![Live view, light theme](docs/img/dashboard.png)

### Running it without Azure

To see the dashboard without deploying anything:

```bash
python tools/mock-server.py
# http://127.0.0.1:8750/index.html
```

That serves `public/` with a stub `/api/data/*` returning fabricated incidents,
so the whole UI — filters, charts, SLA logic, theme toggle — works offline.
